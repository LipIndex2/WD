...
class CodeWriter {
    ...
    write(show_str: string, block = 0): CodeWriter {
        if (block == -1) { this.endBock(); }
        let prefix = "";
        for (let i = 0; i < this.blockNum; i++) {
            prefix += "\t";
        }
        this.towrite(prefix + show_str, 1);
        if (block == 1) { this.startBlock(); }
        return this;
    }

    private towrite(text, pos) {
        if (pos == -1) {
            this.showStr = text + "\n" + this.showStr;
        } else if (pos == 1) {
    ...
    writeImport() {
        ...
        this.fromList.sort((a, b) => {
            let a_path = "";
            a.replace(/\"(.*)\"/, (match: string, ...args: any) => { a_path = match; return match; });
            let b_path = "";
            b.replace(/\"(.*)\"/, (match: string, ...args: any) => { b_path = match; return match; });
    ...
    private btnList: string[] = [];
    addBtn(btnName: string) {
    ...
    copy() {
        FairyEditor.Clipboard.SetText(this.toString());
    }
}

//type:组件名字  from:是否需要引入  button:是否按钮(加click事件)
var componentMod: { [key: string]: { type: string, from?: string, button?: boolean } } = {
    //CommonButton
...
class NodeType {
    static Format(node: FairyGUI.Utils.XML, link_str: string) {
        return `${node.GetAttribute("name")}: ${link_str},`;
    }

    //是否组件
    static IsCommponent(node: FairyGUI.Utils.XML, code: CodeWriter) {
    ...
    //是否文本
    static IsText(node: FairyGUI.Utils.XML, code: CodeWriter) {
        let name = node.GetAttribute("name");
        if (node.name == "text") {
    ...
    //是否富文本
    static IsRichText(node: FairyGUI.Utils.XML, code: CodeWriter) {
        let name = node.GetAttribute("name");
    ...
    static IsList(node: FairyGUI.Utils.XML, code: CodeWriter) {
        if (node.name == "list") {
    ...
    static IsLoader(node: FairyGUI.Utils.XML, code: CodeWriter) {
        if (node.name == "loader") {
...
export class ClientTool {
    private static isXml(): FairyEditor.FPackageItem {
        let lib_view = FairyEditor.App.libView;
    ...
    public static InitView() {
        ...
        let com_asset = new FairyEditor.ComponentAsset(select);
    ...
    public static Rename() {
        let pinyin = new Pinyin({ charCase: 0, checkPolyphone: true });
    ...
    //检测是否为设置字体
    public static CheckFont() {
        let lib_view = FairyEditor.App.libView;
        const select_list = lib_view.GetSelectedResources(false);
        select_list.ForEach(pkg_item => {
            if (pkg_item.fileName.endsWith(".xml")) {
                let com_asset = new FairyEditor.ComponentAsset(pkg_item);
                let dis_node = com_asset.xml.GetNode("displayList");
                if (dis_node == null) {
                    return;
                }
