...
import { ZombieRushPassCtrl } from "./ZombieRushPassCtrl";
...
@BaseView.registView
export class ZombieRushPassView extends BaseView {

    protected viewRegcfg = {
        UIPackName: "ZombieRushPass",
        ViewName: "ZombieRushPassView",
    ...
    listData: { cfg: import("d:/ccs/wjszm-c/assets/script/config/CfgZombieRushPass").CfgZombieRushPassSet; isActive: boolean[]; isFetch: boolean[]; level: number; isShowWire: boolean; }[];

    InitData() {

        this.viewNode.UISpineShow.LoadSpine(ResPath.Spine("jiangshichongchongchong/jiangshichongchongchong_top"), true);

        this.AddSmartDataCare(ZombieRushPassData.Inst().FlushData, this.FulshListData.bind(this), "FlushInfo");
        ...
        this._key_check_buy1 = Mod.ZombieRushPass.View + "-1";
        this._key_check_buy2 = Mod.ZombieRushPass.View + "-2";

        this.FulshListData();

        let scroll = ZombieRushPassData.Inst().scrollListNum();
        this.viewNode.list.scrollToView(scroll);

    }

    FulshListData() {
        let listdata = ZombieRushPassData.Inst().GetListData()
        this.viewNode.list.itemRenderer = this.itemRenderer.bind(this);
        this.listData = listdata;
        this.viewNode.list.numItems = listdata.length;

        this.viewNode.BtnBuy1.visible = !ZombieRushPassData.Inst().GetIsActiveInfo(1);
        this.viewNode.BtnBuy2.visible = !ZombieRushPassData.Inst().GetIsActiveInfo(2);

        // this.viewNode.BtnOneKeyGet.visible = ZombieRushPassData.Inst().GetAllRed() > 0
    ...
    InitUI() {
        let otherCfg = ZombieRushPassData.Inst().GetOtherCfg();
    ...
    private FlushFlushTime() {
        let endTime = ZombieRushPassData.Inst().getEndTime();
    ...
    OnClickBuy(type: number) {
        if (ZombieRushPassData.Inst().GetIsActiveInfo(type)) {
            return;
        }
        ...
        let data = ZombieRushPassData.Inst().GetOtherCfg();
        ...
        let order_data = Order_Data.initOrder(type, ACTIVITY_TYPE.ZombieRushPass, payPrice, payPrice, name);
        OrderCtrl.generateOrder(order_data);
    }

    OnClickOneKeyGet() {
        ZombieRushPassCtrl.Inst().SendOnKeyGet();
    }
...
class RoundListCell extends BaseItem {
    protected viewNode = {
    ...
    public SetData(data: any) {
        ...
        let other = ZombieRushPassData.Inst().GetOtherCfg();
        this.viewNode.BtnBuy.visible = ZombieRushPassData.Inst().GetBuyBtnShow(data.cfg.seq)
    ...
    private OnClickBtnBuy() {
        ZombieRushPassCtrl.Inst().SendBuyLevel();
    }
}
class RoundItem extends BaseItemGB {
    ...
    public SetData(data: any) {
        ...
        UH.SpriteName(this.viewNode.Bg, "ZombieRushPass", "ItemCellBg" + data.type);
        ...
        if (item.item_type == 11) {
            ViewManager.Inst().OpenView(RoundActivityBoxView, {
                ...
                callback: () => {
                    ...
                    ZombieRushPassCtrl.Inst().SendFetchReward(this.data.type, this.data.info.cfg.seq);
                }
            });
            return;
        }
