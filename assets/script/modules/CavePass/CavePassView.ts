...
import { CavePassCtrl } from "./CavePassCtrl";
...
@BaseView.registView
export class CavePassView extends BaseView {

    protected viewRegcfg = {
        UIPackName: "CavePass",
        ViewName: "CavePassView",
    ...
    InitData() {
        this.AddSmartDataCare(CavePassData.Inst().FlushData, this.FulshListData.bind(this), "FlushInfo");
        this.viewNode.UISpineShow.LoadSpine(ResPath.Spine("kuangdongzhanling/kuangdongzhanling"), true);
        ...
        this._key_check_buy = Mod.CavePass.View + "-1";

        let scrollTo = CavePassData.Inst().scrollListNum();
        this.viewNode.List.scrollToView(scrollTo);
    }

    FulshListData() {
        let isActive = CavePassData.Inst().GetIsActive(1)

        this.passData = CavePassData.Inst().GetCavePassList();
        ...
        UH.SetText(this.viewNode.NowadayLevel, TextHelper.Format(Language.CavePass.mine, CavePassData.Inst().Meters));

        this.viewNode.LevelShow.x = isActive ? (800 - this.viewNode.LevelShow.width) / 2 : 45;
        this.viewNode.BtnBuy.visible = !isActive;
        // this.viewNode.BtnOneKeyGet.visible = CavePassData.Inst().GetAllRed() > 0
    ...
    InitUI() {
        let otherCfg = CavePassData.Inst().GetOtherCfg();
    ...
    OnClickOneKeyGet() {
        CavePassCtrl.Inst().SendOnKeyGet();
    }
    ...
    OnClickBuy() {
        ...
        let data = CavePassData.Inst().GetOtherCfg();
        let order_data = Order_Data.initOrder(1, ACTIVITY_TYPE.CavePass, data.pay_price, data.pay_price, Language.CavePass.name);
        OrderCtrl.generateOrder(order_data);
    }

    private FlushTime() {
        let time = ActivityData.Inst().GetEndStampTime(ACTIVITY_TYPE.CavePass) - TimeCtrl.Inst().ServerTime;
    ...
    private GetListItemResource(index: number) {
        ...
        if (data === 0) {
            return fgui.UIPackage.getItemURL("CavePass", "BtnAwardBox");
        } else {
            return fgui.UIPackage.getItemURL("CavePass", "ListAaward");
        }
    }
...
class ListAaward extends BaseItem {
    ...
    private OnClickBtnBuy() {
        CavePassCtrl.Inst().SendBuyLevel();

        // const other = CavePassData.Inst().GetOtherCfg();
    ...
    public SetData(data: any) {
        ...
        let Meters = CavePassData.Inst().Meters;
        ...
        let other = CavePassData.Inst().GetOtherCfg();
        this.viewNode.BtnBuy.visible = CavePassData.Inst().GetBuyBtnShow(data.seq);
...
class PrizeItem extends BaseItemGB {
    ...
    public SetData(data: any) {
        ...
        let IsReward = CavePassData.Inst().IsGetReward(data.type, data.seq);
        let isLock = CavePassData.Inst().GetIsActive(data.type);
    ...
    private OnClickFetchReward() {
        let isGetPrize = CavePassData.Inst().IsGetReward(this.data.type, this.data.seq);
        // if (isGetPrize) {
        //     PublicPopupCtrl.Inst().Center(Language.ActCommon.JiangLiYiLingQu);
        //     return
        // }
        // if (!isReceive) {
        //     PublicPopupCtrl.Inst().Center(Language.CavePass.tip);
        //     return
        // }
        let isActive = CavePassData.Inst().GetIsActive(this.data.type);
        // if (!isActive) {
        //     PublicPopupCtrl.Inst().Center(Language.TrafficPermit.extraAward);
        //     return
        // }
        let isReceive = this.data.level <= CavePassData.Inst().Meters && !isGetPrize && isActive;
        ...
        if (item.item_type == 11) {
            ViewManager.Inst().OpenView(RoundActivityBoxView, {
                ...
                callback: () => {
                    ...
                    CavePassCtrl.Inst().SendFetchReward(this.data.type, this.data.seq);
                }
            });
            return;
        }
...
class BtnAwardBox extends BaseItemGB {
    public SetData() {
        ...
        this.title = CavePassData.Inst().fetchEx + ""
    }
    private OnClickBoxOpen() {
        let otherCfg = CavePassData.Inst().GetOtherCfg();
        let item = Item.GetConfig(otherCfg.max_item.item_id);
        let isReceive = CavePassData.Inst().fetchEx > 0;
        ViewManager.Inst().OpenView(RoundActivityBoxView, {
            item: item,
            isReceive: isReceive,
            callback: () => {
                BagData.Inst().ShowRewardBox = true;
                CavePassCtrl.Inst().SendFetchEx();
            }
        });
    }
}
...
class BtnSkin extends BaseItemGB {
    ...
    public SetData(data: number) {
        let levelCfg = CavePassData.Inst().GetPasscheckLevelCfg(data);
        ...
        UH.SetText(this.viewNode.NextLevel, Language.CavePass.depth + levelCfg.meters);
    }
}