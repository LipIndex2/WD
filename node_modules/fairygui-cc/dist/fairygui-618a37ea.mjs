...
Event.SETDATA = "fui_setdata";
Event.TWEENER_ERROR = "fui_tweener_error";
...
class GTweener {
    ...
    setTotalTimeScale(value) {
        this._totalTimeScale = value;
        return this;
    }
    setDirection(value) {
    ...
    _update(dt) {
        if (this._timeScale != 1)
            dt *= this._timeScale;
        if (this._totalTimeScale != 1)
            dt *= this._totalTimeScale;
        ...
        if (this._ended != 0) //Maybe completed by seek
         {
            this.callCompleteCallback();
            this.autoKill ? this._killed = true : this._paused = true;
            return;
        }
        if (this._paused_num <= 0)
    ...
    callStartCallback() {
        if (this._onStart) {
            try {
                ...
                GRoot.inst.emit(Event.TWEENER_ERROR, err);
            }
        }
    }
    callUpdateCallback() {
        if (this._isError) {
            if (this._isErrorEmit) {
                let stack = new Error().stack;
                GRoot.inst.emit(Event.TWEENER_ERROR, "fgui的 tweener callUpdateCallback报错啦", stack);
                this._isErrorEmit = false;
            }
            return;
        }
        if (this._onUpdate) {
...
class TweenManager {
    static createTween(autoKill = true) {
        if (!_root) {
        ...
        tweener.autoKill = autoKill;
        _activeTweens[_totalActiveTweens++] = tweener;
        TweenManager.tween = tweener;
        return tweener;
    }
...
class GTween {
    static to(start, end, duration, autoKill = true) {
        return TweenManager.createTween(autoKill)._to(start, end, duration);
    }
    static to2(start, start2, end, end2, duration, autoKill = true) {
        return TweenManager.createTween(autoKill)._to2(start, start2, end, end2, duration);
    }
    static to3(start, start2, start3, end, end2, end3, duration, autoKill = true) {
        return TweenManager.createTween(autoKill)._to3(start, start2, start3, end, end2, end3, duration);
    }
    static to4(start, start2, start3, start4, end, end2, end3, end4, duration, autoKill = true) {
        return TweenManager.createTween(autoKill)._to4(start, start2, start3, start4, end, end2, end3, end4, duration);
    }
...
UIConfig.frameTimeForAsyncUIConstruction = 0.02;
...
class GObject {
    ...
    get dataJsonObj() {
        if (this._dataJsonObj) {
            return this._dataJsonObj;
        }
        if (this.data) {
            this._dataJsonObj = JSON.parse(this.data);
            return this._dataJsonObj;
        }
        else {
            return null;
    ...
    set fillMethod(value) {
        if (this._fillMethod != value) {
            ...
            if (this._fillMethod != 0) {
                ...
                if (!this._fillCenterSelf)
                    this.fillCenter = new Vec2(0.5, 0);
                this.setupFill();
            }
            else {
...
class GImage extends GObject {
    ...
    set fillAmount(value) {
        this._content.fillAmount = value * this._barTimes;
    }
    constructFromResource() {
    ...
    checkFill() {
        let customData = this.dataJsonObj;
        if (customData) {
            if (customData["FillCenter"]) {
                let FillCenter = customData["FillCenter"];
                if (FillCenter && FillCenter.x && FillCenter.y) {
                    this._content._fillCenterSelf = true;
                    this._content.fillCenter.set(FillCenter.x, FillCenter.y);
                }
            }
            if (customData["barTimes"]) {
                let barTimes = customData["barTimes"];
...
class ByteBuffer {
    ...
    readLL() {
        this.validate(8);
        let ret = this._view.getBigInt64(this._pos, this.littleEndian);
        this._pos += 8;
        return Number(ret);
    }
    readString(len) {
...
class GTextField extends GObject {
    ...
    ensureSizeCorrect() {
        if (this._sizeDirty) {
            if (this._label.UpdateContentSize) {
                this._label.UpdateContentSize();
            }
            else if (this._label.updateRenderData) {
    ...
    setup_beforeAdd(buffer, beginPos) {
        super.setup_beforeAdd(buffer, beginPos);
        buffer.seek(beginPos, 5);
        var fnt = buffer.readS();
        var fntS = buffer.readShort();
        if (this._label) {
            let customData = this.dataJsonObj;
            let cacheMode;
            if (customData && customData["cacheMode"] != null && customData["cacheMode"] != undefined) {
                cacheMode = customData["cacheMode"];
            }
            else if (fntS >= 30) {
                cacheMode = CacheMode.BITMAP;
            }
            else {
                cacheMode = CacheMode.CHAR;
            }
            // console.error(`name=${this.name},selfData=${this.data},dataJson=${this.dataJsonObj},lable=${this._label},cacheM=${cacheMode}`);
...
class GRichTextField extends GTextField {
    ...
    updateText() {
        ...
        if (this._underline)
            text2 = "<u>" + text2 + "</u>";
        if (this._stroke)
            text2 = `<outline color=#${this._strokeColor.toHEX("#rrggbb")} width=${this._stroke}>${text2}</outline>`;
        let c = this._color;
        if (this._grayed)
            c = toGrayedColor(c);
        ...
        //     // if(this._richText.)
        //     // let rich = this._richText;
        //     // (rich as any)._segments.forEach((element: { comp: { overflow: AutoSizeType; }; }) => {
        //     //     element.comp.overflow = this._autoSize - 1;
        //     // })
        // }
    }
    updateFont() {
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        this._richText.cacheMode = cacheMode;
    }
    set stroke(value) {
        this._stroke = value;
    }
    ...
    updateStrokeColor() {
        if (!this._strokeColor)
            this._strokeColor = new Color();
        if (this._grayed)
            this._strokeColor = toGrayedColor(this._strokeColor);
    }
}
...
class InputProcessor extends Component {
    ...
    touchEndHandler(evt) {
        let ti = this.updateInfo(evt.getID(), evt.getLocation(), 2);
        if (!this._touchListener) {
            let e = evt;
            e.preventSwallow = (ti.target == this._owner);
        }
...
class ScrollPane extends Component {
    ...
    setup(buffer) {
        ...
        if ((flags & 512) == 0) {
            let mask = this._maskContainer.addComponent(Mask);
            let sp = this._maskContainer.addComponent(Sprite);
            let spf = builtinResMgr.get('default-spriteframe');
            sp.spriteFrame = spf;
    ...
    onTouchMove(evt) {
        ...
        var deltaTime = Math.max(now - this._lastMoveTime, 1 / game.totalTime);
...
class GComponent extends GObject {
    constructor() {
        ...
        this._apexIndex = 0;
        this._hitBySiblingIndex = false;
        this._node.name = this.constructor.name;
    ...
    onChildAdd(child, index) {
        ...
        if (this._childrenRenderOrder == ChildrenRenderOrder.Ascent)
            ...
            //if (NATIVE) {
            let cindex = cnt - index - 1;
            child.node.setSiblingIndex(cindex);
            //} else
            // child.node.setSiblingIndex(-index);
        }
        else
    ...
    _hitTest(pt, globalPt) {
        ...
        if (this._hitBySiblingIndex) {
            let sibling_sort = [];
            for (let i = 0; i < cnt; i++) {
                sibling_sort.push(i);
            }
            sibling_sort.sort((a, b) => {
                return this._children[b].node.getSiblingIndex() - this._children[a].node.getSiblingIndex();
            });
            for (let i = 0; i < sibling_sort.length; i++) {
                if (check_fun(sibling_sort[i])) {
                    break;
                }
            }
        }
...
class GRoot extends GComponent {
    ...
    showPopup(popup, target, dir) {
        ...
        let pos = this.globalToLocal(pt.x, pt.y);
...
class GLoader extends GObject {
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        if (buffer.readBool())
            this.color = buffer.readColor();
        this._data_content[0] = buffer.readByte();
        if (this._data_content[0] != 0) {
            this._data_content[1] = buffer.readByte();
            this._data_content[2] = buffer.readByte();
            this._data_content[3] = buffer.readByte();
        }
        if (this._url)
            this.loadContent();
    }
    set_content() {
        if (this._data_content && this._data_content.length) {
            this._content.fillMethod = this._data_content[0];
            if (this._data_content[0] != 0) {
                this._content.fillOrigin = this._data_content[1];
                this._content.fillClockwise = this._data_content[2];
                this._content.fillAmount = this._data_content[3];
            }
        }
    }
}
...
class GLoader3D extends GObject {
    ...
    setDragonBones(asset, atlasAsset, anchor, pma) {
        ...
        this._content.color = this._color;
        let armatureKey = asset["init"](__private._cocos_dragon_bones_CCFactory__CCFactory.getInstance(), atlasAsset["_uuid"]);
...
class GList extends GComponent {
    ...
    OnSelectedItem(selectedIndex) {
        ...
        if (item == null || item == undefined)
            return;
        if (this.node.hasEventListener(Event.CLICK_ITEM)) {
            this.node.emit(Event.CLICK_ITEM, item);
        }
    }
    SetData(data, target, selIndex) {
        ...
        if (target != null) {
            if (!this.node.hasEventListener(Event.CLICK_ITEM)) {
                if (typeof (target) == "function") {
                    this.on(Event.CLICK_ITEM, target);
                }
                else {
                    this.on(Event.CLICK_ITEM, target.OnClickItem, target);
                }
            }
            if (this._selectionMode == ListSelectionMode.Single) {
                this.OnSelectedItem(selIndex !== null && selIndex !== void 0 ? selIndex : 0);
            }
        }
        this.updateBounds(); //设置数据的同帧刷新物体的位置，防止列表表现上的闪动
        this._node.emit(Event.SETDATA);
    }
}
var s_n = 0;
...
class GTree extends GList {
    ...
    createCell(node) {
        ...
        if (cc)
            cc.selectedIndex = node.level - 1; //node.isFolder ? 0 : 1;
        if (node.isFolder)
